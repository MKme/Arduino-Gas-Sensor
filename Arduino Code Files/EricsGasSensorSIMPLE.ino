...
V6
Removed discreet level outputs of the 3 gasses because the MQ2 has a hard time measuring the remaining 2 when flooded with one gas
Having a single Go/No-Go indicator is safer than incorrect level reporting IMO
A new sensor suite is needed to measure more levels and measure them accurately
Tried everything I can think of to make the discreet results work- no joy. Sensor ccan show smoke and CO when satured in LPG and may be true vice versa.
Untill new sensor is found- simple code is better to treat area as worst case- all 3 levels present.
...
void loop() {
  ...
  delay(50);  //Give the analog read time to settle with the basic go-nogo code
  ...
  delay(50);  //Give the analog read time to settle with the basic go-nogo code
   ...
   Serial.print( "ppm" );
   Serial.print("\n");
   if ((co !=0) or (lpg !=0) or (smoke !=0)) 
 {voice.say(spALERT); 
  voice.say(spALERT);
  voice.say(spALERT);
}
 
   else voice.say(spSAFE);
  //voice.say(spBREAK);
